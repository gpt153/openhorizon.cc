...
import { UtensilsCrossed, Loader2, CheckCircle2, Mail } from 'lucide-react'
...
export function FoodSearchPanel({
  ...
  const searchMutation = trpc.pipeline.phases.searchFood.useMutation({
    onSuccess: (data) => {
      setSearchResults(data.options)
      toast.success('Found food options!')
    },
    onError: (error) => {
